...
// Image CDN base URL - empty for local, set to R2 domain for production
...
function scanPhotosInFolder(folderPath, urlPrefix) {
  ...
  for (const entry of entries) {
    if (entry.isFile() && isImageFile(entry.name)) {
      ...
      photos.push({
        src: `${IMAGE_BASE_URL}${urlPrefix}/${entry.name}`,
...
export const getSiteData = cache(async function getSiteData() {
  ...
  const categories = CATEGORIES.map((slug) => {
    ...
    // Prefix coverPhoto with IMAGE_BASE_URL if it exists
