...
function getPhotosInFolder(folderPath) {
  ...
  const photos = manifest
    ...
    .map(item => {
      ...
      // Generate thumb path (same structure, .jpg extension, in /thumbs/)
      const baseName = item.Name.replace(/\.[^.]+$/, ".jpg");
      const thumbPath = item.Path.replace(item.Name, baseName);

      return {
        src: `${IMAGE_BASE_URL}/photos/${item.Path}`,
        thumb: `${IMAGE_BASE_URL}/thumbs/${thumbPath}`,
